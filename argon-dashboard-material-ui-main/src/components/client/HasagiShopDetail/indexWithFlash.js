...
function ShopDetailFS() {
    ...
    const fetchProductDetail = async () => {
        ...
        try {
            ...
            const response = await cartService.getProductDetailFS({
                productId,
    ...
    const fetchPrice = async (productId, colorId, sizeId) => {
        try {
            ...
            const { price, quantity: maxQuantity } = response.data;
            setTotalPrice(price);
            setQuantityPDT(maxQuantity);
    ...
    const handleByNow = async () => {
        try {
            ...
            if (response && response.data && response.data.id) {
                localStorage.setItem('lastAddedItemId', response.data.id);
                navigate("/Cart");
            } else {
                console.log("Failed to add to cart or missing ID:", response);
            }
                                ...
                                <div className="input-group quantity mr-3" style={{ width: "130px" }}>
                                    <div className="input-group-btn">
                                        <button
                                            ...
                                            disabled={quantity <= 1 || !selectedColor || !selectedSize} 
                                    ...
                                    <input
                                        ...
                                        onBlur={() => {
                                            if (quantity > quantityPDT) {
                                                setQuantity(quantityPDT);
                                    
                                               
                                                Swal.fire({
                                                    title: "Thông báo",
                                                    text: `Số lượng vượt quá giới hạn. Đã giảm về mức tối đa là ${quantityPDT}.`,
                                                    icon: "warning",
                                                    showClass: {
                                                        popup: `
                                                            animate__animated
                                    ...
                                    <div className="input-group-btn">
                                        <button
                                            ...
                                            onClick={() => setQuantity(quantity + 1)}
                                            disabled={!selectedColor || !selectedSize ||quantity === quantityPDT} 
