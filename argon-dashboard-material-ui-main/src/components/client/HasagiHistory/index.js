...
const History = () => {
  ...
  useEffect(() => {
    const fetchOrderHistory = async () => {
      try {
        const response = await HistoryOrderService.getHistory(); // Call the corrected method
        console.log("Fetched Order History:", response); 

        // Ensure response is an array
        if (Array.isArray(response.data)) { // Assuming response.data contains the order history
